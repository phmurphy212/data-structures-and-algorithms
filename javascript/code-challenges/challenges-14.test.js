...
const screenForNames = (arr) => {
  ...
  arr.map(word => {
    ...
    regex.test(word) ? newArr.push(word) : '';
  });
  return newArr;
};
...
// code challenge 11 number 5

let biggerThanLuke = (arr) => {
  return arr.filter((jedi) => jedi.mass > 77).map((jedi) => jedi.name).join(' - ');
};
...
------------------------------------------------------------------------------------------------ */
// I understand all of this one except the 1 : -1 part which my colleague just told me to do and it worked
