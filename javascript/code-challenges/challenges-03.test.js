...
const addTwo = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++){
    let twoAdded = arr[i] + 2;
    newArr.push(twoAdded);
  }
  return newArr;
};
...
const containsAnd = (arr) => {
  let hasAnd = arr.filter(hasAnd => hasAnd.includes('and'));
  return hasAnd;
};
...
const oddValues = (arr) => {
  let isOdd = arr.filter(odd => odd%2);
  return isOdd;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let notFirst = arr.filter(second => !forbiddenValues.includes(second));
  return notFirst;
};
